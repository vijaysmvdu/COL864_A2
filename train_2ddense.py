...
# added by me to preprocess liver data 
def load_seq_crop_data_maskliver_try(Parameter_List):
    img = Parameter_List[0]
    liver = Parameter_List[1]
    lines = Parameter_List[2]
    numid = Parameter_List[3]
    minindex = Parameter_List[4]
    maxindex = Parameter_List[5]
    ...
    cropp_img = img[a - deps // 2:a + deps // 2, b - rows // 2:b + rows // 2,
                c - cols // 2: c + cols // 2 + 1].copy()
    cropp_liver = liver[a - deps // 2:a + deps // 2, b - rows // 2:b + rows // 2,
                  c - cols // 2:c + cols // 2 + 1].copy()
    ...
    if flip_num == 1:
        cropp_img = np.flipud(cropp_img)
        cropp_liver = np.flipud(cropp_liver)
    elif flip_num == 2:
        cropp_img = np.fliplr(cropp_img)
        cropp_liver = np.fliplr(cropp_liver)
    elif flip_num == 3:
        cropp_img = np.rot90(cropp_img, k=1, axes=(1, 0))
        cropp_liver = np.rot90(cropp_liver, k=1, axes=(1, 0))
    elif flip_num == 4:
        cropp_img = np.rot90(cropp_img, k=3, axes=(1, 0))
        cropp_liver = np.rot90(cropp_liver, k=3, axes=(1, 0))
    elif flip_num == 5:
        cropp_img = np.fliplr(cropp_img)
        cropp_liver = np.fliplr(cropp_liver)
        cropp_img = np.rot90(cropp_img, k=1, axes=(1, 0))
    ...
    cropp_liver = resize(cropp_liver, (args.input_size,args.input_size,args.input_cols), order=0, mode='edge', cval=0, clip=True, preserve_range=True)
    cropp_img   = resize(cropp_img, (args.input_size,args.input_size,args.input_cols), order=3, mode='constant', cval=0, clip=True, preserve_range=True)
    return cropp_img, cropp_liver[:,:,1]
...
def generate_arrays_from_file_liver(batch_size, trainidx, img_list, tumor_list, tumorlines, liverlines, tumoridx, liveridx, minindex_list, maxindex_list):
    while 1:
        X = np.zeros((batch_size, args.input_size, args.input_size, args.input_cols), dtype='float32')
        Y = np.zeros((batch_size, args.input_size, args.input_size, 1), dtype='int16')
        Parameter_List = []
        for idx in range(batch_size):
            ...
            Parameter_List.append([img, tumor, lines, numid, minindex, maxindex])
        pool = ThreadPool(thread_num)
        result_list = pool.map(load_seq_crop_data_maskliver_try, Parameter_List)
        pool.close()
        pool.join()
        for idx in range(len(result_list)):
            X[idx, :, :, :] = result_list[idx][0]
...
def load_fast_files(args):
    ...
    trainidx = list(range(0,4))
    #trainidx = list(range(0,131))
    img_list = []
    tumor_list = [] # it is same for liver and tumor
    #liver_list = []
    minindex_list = []
    maxindex_list = []
    tumorlines = []
...
def train_and_predict():
   ...
   # model_checkpoint = ModelCheckpoint(args.save_path + 'model/weights.{epoch:02d}-{loss:.2f}.hdf5', monitor='loss', verbose = 1,save_best_only=False,save_weights_only=False,mode = 'min', period = 1)
    ...
    steps = 27386//args.b
    model.fit_generator(generate_arrays_from_file(args.b, trainidx, img_list, tumor_list, tumorlines, liverlines, tumoridx, liveridx, minindex_list, maxindex_list),steps_per_epoch=steps, epochs= 5, verbose = 1, callbacks = [model_checkpoint], max_queue_size=10, workers=1, use_multiprocessing=False)
    # orignal epoch=6000

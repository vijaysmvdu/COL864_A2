...
def generate_livertxt(image_path, save_folder):
    ...
    for i in range(0,28): # old one is range(0,131)
...
def generate_txt(image_path, save_folder):
    ...
    for i in range(0,28): # old one is range(0,131)
        values = np.loadtxt('data/myTestDataTxt/LiverPixels/liver_' + str(i) + '.txt', delimiter=' ', usecols=[0, 1, 2])
        a = np.min(values, axis=0)
        b = np.max(values, axis=0)
        box = np.append(a,b, axis=0)
        np.savetxt('data/myTestDataTxt/LiverBox/box_'+str(i)+'.txt', box,fmt='%d')


#proprecessing(image_path='resultstest/', save_folder='results/')
#proprecessing(image_path='data/TestData/', save_folder='myTestDataTxt/')
print ("Generate liver txt ")
generate_livertxt(image_path='data/TestData/', save_folder='myTestDataTxt/')
print ("Generate tumor txt")
generate_tumortxt(image_path='data/TestData/', save_folder='myTestDataTxt/')
print ("Generate liver box ")
generate_txt(image_path='data/TestData/', save_folder='myTestDataTxt/')

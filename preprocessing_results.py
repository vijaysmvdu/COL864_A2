...
def generate_livertxt(image_path, save_folder):
    ...
    for i in range(2,28): # old one is range(0,131)
        livertumor, header = load(image_path+'resultstest-segmentation-'+str(i)+'.nii')
...
def generate_txt(image_path, save_folder):
    ...
    for i in range(2,28): # old one is range(0,131)
        values = np.loadtxt('data/results/LiverPixels/liver_' + str(i) + '.txt', delimiter=' ', usecols=[0, 1, 2])
        a = np.min(values, axis=0)
        b = np.max(values, axis=0)
        box = np.append(a,b, axis=0)
        np.savetxt('data/results/LiverBox/box_'+str(i)+'.txt', box,fmt='%d')
...
generate_livertxt(image_path='resultstest/', save_folder='results/')
print ("Generate tumor txt")
generate_tumortxt(image_path='resultstest/', save_folder='results/')
print ("Generate liver box ")
generate_txt(image_path='resultstest/', save_folder='results/')
